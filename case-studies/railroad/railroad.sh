...
for K in {1..20}
do
	java -Xmx8g -Djava.library.path=/usr/local/lib -cp bin/.:lib/* main.Main -ltl="G((t && X(c)) || (! t && X(! c))) && G(X(b) -> (a && g)) && G (c -> ! b) && ! G (t -> ! g)" -bc="(((! b && (g && t)) || (b && ((! g && c) || (g && (t || c))))))" -alph="[a,b,c,g,t]" -k=$K -out=case-studies/railroad/RAILROAD-DOM-BC1-G1.txt
done

echo "Domain and BC1 and not G1 and G2"
for K in {1..20}
do
	java -Xmx8g -Djava.library.path=/usr/local/lib -cp bin/.:lib/* main.Main -ltl="G((t && X(c)) || (! t && X(! c))) && G(X(b) -> (a && g)) && ! G (c -> ! b) && G (t -> ! g)" -bc="(((! b && (g && t)) || (b && ((! g && c) || (g && (t || c))))))" -alph="[a,b,c,g,t]" -k=$K -out=case-studies/railroad/RAILROAD-DOM-BC1-G2.txt
done
